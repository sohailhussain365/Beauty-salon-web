        ...
        <div key={label} className="flex items-center">
          <div className="flex flex-col items-center gap-1.5">
            <div
              ...
              style={{
                background: i <= step ? "hsl(22,15%,12%)" : "rgba(22,15,8,0.06)",
                border: i === step ? "2px solid hsl(22,15%,12%)" : i < step ? "2px solid transparent" : "2px solid rgba(22,15,8,0.12)",
                boxShadow: i === step ? "0 0 16px rgba(22,15,8,0.18)" : "none",
              }}>
              {i < step ? (
                ...
                <span style={{ fontSize: 11, fontWeight: 600, color: i <= step ? "#fff" : "rgba(22,15,8,0.30)" }}>{i + 1}</span>
              )}
            </div>
            <span className="hidden sm:block transition-colors duration-300"
              style={{ fontSize: 8.5, letterSpacing: "0.25em", textTransform: "uppercase", color: i <= step ? "rgba(22,15,8,0.60)" : "rgba(22,15,8,0.22)" }}>
              {label}
            </span>
          </div>
          {i < STEP_LABELS.length - 1 && (
            <div className="w-12 md:w-20 h-px mx-2 mb-5 transition-all duration-700 relative overflow-hidden">
              <div className="absolute inset-0" style={{ background: "rgba(22,15,8,0.08)" }} />
              <motion.div
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 pb-16">
      <div className="text-center mb-10">
        <p style={{ fontSize: 13, color: "rgba(22,15,8,0.45)", lineHeight: 1.6 }}>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                border: `1px solid ${isSelected ? "hsl(22,15%,14%)" : "rgba(22,15,8,0.08)"}`,
                boxShadow: isSelected ? "0 0 0 2px hsl(22,15%,14%), 0 8px 24px rgba(22,15,8,0.10)" : "0 2px 10px rgba(22,15,8,0.04)",
              }}>
              ...
              <div className="relative z-10">
                <span className="text-2xl mb-4 block transition-colors duration-300"
                  style={{ color: isSelected ? "hsl(22,15%,14%)" : "rgba(22,15,8,0.30)" }}>{svc.icon}</span>
                <h3 className="font-serif font-semibold mb-1.5 transition-colors duration-300"
                  style={{ fontSize: 13.5, color: isSelected ? "hsl(22,20%,8%)" : "hsl(22,20%,12%)" }}>
                  {svc.name}
                </h3>
                <p style={{ fontSize: 11, color: "rgba(22,15,8,0.42)", lineHeight: 1.6, marginBottom: 12 }}>{svc.desc}</p>
                <div className="flex items-center justify-between">
                  <span style={{ fontSize: 9, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(22,15,8,0.28)" }}>{svc.duration}</span>
                  <span className="font-semibold" style={{ fontSize: 11, color: "rgba(22,15,8,0.55)" }}>{svc.price}</span>
                </div>
              </div>
            </motion.button>
          );
      ...
      <div className="flex justify-center mt-10">
        <motion.button
          ...
          style={{
            background: selected ? "hsl(22,15%,12%)" : "rgba(22,15,8,0.06)",
            border: selected ? "none" : "1px solid rgba(22,15,8,0.10)",
            cursor: selected ? "pointer" : "not-allowed",
            boxShadow: selected ? "0 4px 24px rgba(22,15,8,0.18)" : "none",
          }}
          disabled={!selected}>
          <span style={{ fontSize: 10, letterSpacing: "0.28em", textTransform: "uppercase", color: selected ? "#fff" : "rgba(22,15,8,0.28)", fontWeight: 600 }}>
...
function Calendar({ value, onChange }: { value: string | null; onChange: (d: string) => void }) {
  ...
  const prevMonth = () => { if (viewMonth === 4) return; setViewMonth(m => m - 1); };
  const nextMonth = () => { if (viewMonth === 7) return; setViewMonth(m => m + 1); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <button onClick={prevMonth} className="w-8 h-8 flex items-center justify-center transition-colors"
          style={{ color: viewMonth === 4 ? "rgba(22,15,8,0.15)" : "rgba(22,15,8,0.45)" }}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <span className="font-serif font-medium" style={{ fontSize: 15, color: "rgba(22,15,8,0.75)" }}>
          {MONTH_NAMES[viewMonth]} {viewYear}
        </span>
        <button onClick={nextMonth} className="w-8 h-8 flex items-center justify-center transition-colors"
          style={{ color: viewMonth === 7 ? "rgba(22,15,8,0.15)" : "rgba(22,15,8,0.45)" }}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>
      <div className="grid grid-cols-7 mb-2">
        {DAY_LABELS.map(d => (
          <div key={d} className="text-center py-1" style={{ fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: d === "Su" ? "rgba(22,15,8,0.20)" : "rgba(22,15,8,0.35)" }}>
            {d}
          </div>
        ))}
      </div>
        ...
        {cells.map((day, idx) => {
          ...
          return (
            <button
              ...
              className="aspect-square flex items-center justify-center text-sm transition-all duration-200 relative group"
              style={{
                fontSize: 12,
                fontWeight: isSelected ? 700 : 400,
                cursor: available ? "pointer" : "not-allowed",
                background: isSelected ? "hsl(22,15%,12%)" : "transparent",
                color: isSelected ? "#fff" : isSunday || isPast ? "rgba(22,15,8,0.18)" : available ? "rgba(22,15,8,0.72)" : "rgba(22,15,8,0.22)",
                boxShadow: isSelected ? "0 4px 12px rgba(22,15,8,0.20)" : "none",
              }}>
              {!isSelected && available && (
                <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                  style={{ background: "rgba(22,15,8,0.07)" }} aria-hidden="true" />
              )}
              <span className="relative z-10">{day}</span>
              {available && !isSelected && (
                <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full"
                  style={{ background: "rgba(22,15,8,0.28)" }} aria-hidden="true" />
              )}
            </button>
          );
        })}
...
function DateTimeStep({ date, time, onDateChange, onTimeChange, onNext, onPrev }:
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 pb-16">
      <div className="text-center mb-10">
        <p style={{ fontSize: 13, color: "rgba(22,15,8,0.45)" }}>Select your preferred date and time. Available slots are shown with a dot.</p>
      </div>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="p-7" style={{ background: "#ffffff", border: "1px solid rgba(22,15,8,0.08)", boxShadow: "0 2px 14px rgba(22,15,8,0.04)" }}>
          <p style={{ fontSize: 9, letterSpacing: "0.35em", textTransform: "uppercase", color: "rgba(22,15,8,0.45)", marginBottom: 16 }}>Select Date</p>
          <Calendar value={date} onChange={onDateChange} />
        </div>
        <div className="p-7" style={{ background: "#ffffff", border: "1px solid rgba(22,15,8,0.08)", boxShadow: "0 2px 14px rgba(22,15,8,0.04)" }}>
          <p style={{ fontSize: 9, letterSpacing: "0.35em", textTransform: "uppercase", color: "rgba(22,15,8,0.45)", marginBottom: 16 }}>Select Time</p>
          {!date ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3">
              <div className="w-12 h-12 flex items-center justify-center" style={{ border: "1px solid rgba(22,15,8,0.15)" }}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "rgba(22,15,8,0.35)" }}>
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
              </div>
              <p style={{ fontSize: 12, color: "rgba(22,15,8,0.32)", textAlign: "center" }}>Please select a date first</p>
            </div>
          ) : (
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: isSelected ? "hsl(22,15%,12%)" : isBooked ? "rgba(22,15,8,0.02)" : "rgba(22,15,8,0.04)",
                      border: `1px solid ${isSelected ? "hsl(22,15%,12%)" : isBooked ? "rgba(22,15,8,0.06)" : "rgba(22,15,8,0.12)"}`,
                      color: isSelected ? "#fff" : isBooked ? "rgba(22,15,8,0.20)" : "rgba(22,15,8,0.62)",
                      fontWeight: isSelected ? 600 : 400,
                      textDecoration: isBooked ? "line-through" : "none",
                      boxShadow: isSelected ? "0 4px 12px rgba(22,15,8,0.15)" : "none",
                    }}>
                    {!isSelected && !isBooked && (
                      <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "rgba(22,15,8,0.06)" }} aria-hidden="true" />
                    )}
          ...
          {date && !slots.some(s => !s.isBooked) && (
            <p className="mt-4 text-center" style={{ fontSize: 11, color: "rgba(22,15,8,0.30)" }}>No availability — please choose another date.</p>
          )}
        </div>
      </div>
      <div className="flex items-center justify-between mt-8">
        <button onClick={onPrev} className="flex items-center gap-2 transition-colors"
          style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(22,15,8,0.38)" }}>
        ...
        <button
          ...
          style={{
            background: date && time ? "hsl(22,15%,12%)" : "rgba(22,15,8,0.06)",
            fontSize: 10, letterSpacing: "0.26em", textTransform: "uppercase",
            color: date && time ? "#fff" : "rgba(22,15,8,0.28)",
            cursor: date && time ? "pointer" : "not-allowed",
            boxShadow: date && time ? "0 4px 22px rgba(22,15,8,0.18)" : "none",
          }}>
          Continue
        </button>
      </div>
...
function InfoStep({ form, onChange, onNext, onPrev }:
  ...
  return (
    <div className="max-w-xl mx-auto px-5 pb-16">
      <div className="text-center mb-10">
        <p style={{ fontSize: 13, color: "rgba(22,15,8,0.45)" }}>Almost there. Fill in your details and we'll confirm your appointment.</p>
      </div>
      <div className="p-8 md:p-10" style={{ background: "#ffffff", border: "1px solid rgba(22,15,8,0.08)", boxShadow: "0 2px 14px rgba(22,15,8,0.04)" }}>
        <div className="flex flex-col gap-8">
          {fields.map(f => (
            <div key={f.key} className="relative group">
              <label style={{ fontSize: 8.5, letterSpacing: "0.32em", textTransform: "uppercase", color: "rgba(22,15,8,0.50)", display: "block", marginBottom: 8 }}>
                {f.label} {f.required && <span style={{ color: "rgba(22,15,8,0.38)" }}>*</span>}
              </label>
              <input
                type={f.type}
                ...
                style={{
                  ...
                  borderBottom: `1px solid ${errors[f.key] ? "rgba(200,60,60,0.5)" : "rgba(22,15,8,0.18)"}`,
                }}
                onFocus={e => { e.target.style.borderBottomColor = "hsl(22,15%,18%)"; }}
                onBlur={e => { e.target.style.borderBottomColor = errors[f.key] ? "rgba(200,60,60,0.5)" : "rgba(22,15,8,0.18)"; }}
              />
              {errors[f.key] && (
                <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} style={{ fontSize: 10, color: "rgba(200,60,60,0.80)", marginTop: 5 }}>
                  {errors[f.key]}
                </motion.p>
              )}
            </div>
          ...
          <div className="relative">
            <label style={{ fontSize: 8.5, letterSpacing: "0.32em", textTransform: "uppercase", color: "rgba(22,15,8,0.50)", display: "block", marginBottom: 8 }}>
              Special Requests <span style={{ color: "rgba(22,15,8,0.25)" }}>(optional)</span>
            </label>
            <textarea
              rows={3}
              value={form.notes}
              ...
              style={{ fontSize: 13, color: "rgba(22,15,8,0.62)", borderBottom: "1px solid rgba(22,15,8,0.14)" }}
              onFocus={e => { e.target.style.borderBottomColor = "hsl(22,15%,18%)"; }}
              onBlur={e => { e.target.style.borderBottomColor = "rgba(22,15,8,0.14)"; }}
            />
          </div>
          <p style={{ fontSize: 10, color: "rgba(22,15,8,0.28)", lineHeight: 1.6 }}>
      ...
      <div className="flex items-center justify-between mt-8">
        ...
        <motion.button
          ...
          style={{
            ...
            fontSize: 10, letterSpacing: "0.26em", textTransform: "uppercase", color: "#fff", fontWeight: 600,
            boxShadow: "0 4px 22px rgba(22,15,8,0.20)",
          }}>
          Confirm Appointment
        </motion.button>
      </div>
  ...
  return (
    <div ref={ref} className="max-w-xl mx-auto px-5 pb-20 text-center">
      <motion.div
        ...
        className="mx-auto w-20 h-20 flex items-center justify-center mb-8"
        style={{ background: "hsl(22,15%,12%)", boxShadow: "0 0 60px rgba(22,15,8,0.22)" }}>
        <motion.svg
      ...
      <motion.div
        ...
        <p style={{ fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: "rgba(22,15,8,0.48)", marginBottom: 12 }}>Appointment Confirmed</p>
        <h2 className="font-serif mb-2" style={{ fontSize: 28, color: "hsl(22,20%,8%)", fontWeight: 500 }}>
          You're all set, {name.split(" ")[0]}!
        </h2>
        <p style={{ fontSize: 12.5, color: "rgba(22,15,8,0.40)", marginBottom: 32 }}>
          We'll send a confirmation to your email. We look forward to seeing you.
        </p>

        <div className="p-8 mb-8 text-left" style={{ background: "#ffffff", border: "1px solid rgba(22,15,8,0.10)", boxShadow: "0 2px 14px rgba(22,15,8,0.05)" }}>
          <div className="flex items-start justify-between mb-6 pb-5" style={{ borderBottom: "1px solid rgba(22,15,8,0.08)" }}>
            <div>
              <p style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "rgba(22,15,8,0.42)", marginBottom: 6 }}>Service</p>
              <p className="font-serif font-semibold" style={{ fontSize: 16, color: "hsl(22,20%,8%)" }}>{service.name}</p>
              <p style={{ fontSize: 11, color: "rgba(22,15,8,0.38)", marginTop: 3 }}>{service.duration} · {service.price}</p>
            </div>
            <span className="text-2xl" style={{ color: "rgba(22,15,8,0.30)" }}>{service.icon}</span>
          </div>
          {[
            ...
            <div key={row.label} className="flex items-start gap-4 mb-4 last:mb-0">
              <p style={{ fontSize: 9, letterSpacing: "0.28em", textTransform: "uppercase", color: "rgba(22,15,8,0.38)", width: 72, flexShrink: 0, paddingTop: 2 }}>{row.label}</p>
              <p style={{ fontSize: 12.5, color: "rgba(22,15,8,0.62)", lineHeight: 1.5 }}>{row.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a
            ...
            style={{ fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", color: "rgba(22,15,8,0.55)", border: "1px solid rgba(22,15,8,0.18)" }}>
            Add to Calendar
          </a>
          <Link href="/">
            <button
              className="px-8 py-3.5 font-medium transition-all duration-300 hover:scale-[1.02] text-white"
              style={{ background: "hsl(22,15%,12%)", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", fontWeight: 600 }}>
              Return Home
            </button>
          </Link>
        </div>
...
export default function BookingPage() {
  ...
  return (
    <Layout>
      <div className="min-h-screen relative overflow-hidden" style={{ paddingTop: "70px", background: "hsl(30,8%,93%)" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 100% 60% at 50% 0%, rgba(22,15,8,0.03), transparent 60%)" }} aria-hidden="true" />

        <div className="relative z-10 text-center py-12 px-5">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="flex items-center justify-center gap-4 mb-4">
              <span className="w-10 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(22,15,8,0.22))" }} />
              <span style={{ fontSize: 9, letterSpacing: "0.42em", textTransform: "uppercase", color: "rgba(22,15,8,0.42)" }}>Matthew Dillard Hair Salons</span>
              <span className="w-10 h-px" style={{ background: "linear-gradient(90deg, rgba(22,15,8,0.22), transparent)" }} />
            </div>
            <h1 className="font-serif" style={{ fontSize: "clamp(28px, 4vw, 46px)", color: "hsl(22,20%,8%)", fontWeight: 500 }}>
        ...
        <div className="w-full h-px mb-12" style={{ background: "linear-gradient(90deg, transparent, rgba(22,15,8,0.10), transparent)" }} />
